...
class JsonOutlineProvider {
	...
	init() {
		...
		this.reLoadConfig();
		this.uerLanguage = vscode.env.language;
		vscode.window.showInformationMessage(this.uerLanguage);
		vscode.workspace.onDidChangeConfiguration(this.reLoadConfig);
	}

	reLoadConfig() {
		this.config = vscode.workspace.getConfiguration('fileStructure');
	}
